...
void PassiveControl::computeTorqueCmd(){
    ...
    if (first){
        ...
        static const auto runOnceFirst = [] { std::cout << "going to the first pose" << std::endl; return true;}();       
    }else{
        // ROS_INFO_ONCE("Tracking in process");
        static const auto runOnceTracking = [] { std::cout << "Tracking in process" << std::endl; return true;}(); 
